...
#define PWM_INTERVAL 40

#define STEER_PIN 1 //p1  // 舵机引脚
#define STEER1_PIN 23  // p23
#define STEER2_PIN 26
#define STEER3_PIN 24
...
int g_s2_stop = 1;
int g_s3_stop = 1;
int g_s1_last_duty = 190;
int g_s2_last_duty = 190;
...
#define MOTORB_PIN_1 4    // 马达B引脚1
#define MOTORB_PIN_2 5    // 马达B引脚2
#define MOTORC_PIN_1 25   // 马达C引脚1
#define MOTORC_PIN_2 6   // 马达C引脚2
#define MOTORD_PIN_1 27   // 马达D引脚1
#define MOTORD_PIN_2 28   // 马达D引脚2

int g_control_mode = 0;   // 0 for CS, 1 for AKM, 2 for MKNM
...
int raspi_init()
{
    ...
    pinMode(STEER1_PIN, PWM_OUTPUT); // 设置PWM输出
    pwmSetMode(PWM_MODE_MS); // 设置传统模式
	pwmSetClock(192); // 设置分频
    pwmSetRange(2000); // 设置周期分为2000步

    pinMode(STEER2_PIN, PWM_OUTPUT); // 设置PWM输出
    pwmSetMode(PWM_MODE_MS); // 设置传统模式
	pwmSetClock(192); // 设置分频
    pwmSetRange(2000); // 设置周期分为2000步

    //pinMode(STEER3_PIN, PWM_OUTPUT); // 设置PWM输出
    //pwmSetMode(PWM_MODE_MS); // 设置传统模式
	//pwmSetClock(192); // 设置分频
    //pwmSetRange(2000); // 设置周期分为2000步
...
void steer1_down()
{
    ...
    while (!g_s1_stop && i >= 130) {
        //softPwmWrite(STEER1_PIN, i);
		pwmWrite(STEER1_PIN, i);
		delay(20);
        --i;
    }
    g_s1_last_duty = i;
...
void steer2_down()
{
    ...
    while (!g_s2_stop && i <= 220) {
        //softPwmWrite(STEER2_PIN, i);
		pwmWrite(STEER2_PIN, i);
		delay(20);
        ++i;
    }
    g_s2_last_duty = i;
